  ...
  PPController()
    ...
    state_subscriber_ =
        this->create_subscription<sim_backend::msg::VehicleState>(
            "vehicle_state", 1,
            std::bind(&PPController::state_update, this,
                      std::placeholders::_1));
    ...
    ref_path_subscriber_ =
        this->create_subscription<sim_backend::msg::Point2DArray>(
            ...
            std::bind(&PPController::ref_path_update, this,
                      std::placeholders::_1));
    ...
    control_cmd_timer_ =
        rclcpp::create_timer(this, this->get_clock(), this->dt_,
                             std::bind(&PPController::control_callback, this));
...
private:
  void control_callback() {
    ...
    fx = this->K_P_v_ * this->e_v_ + this->K_I_v_ * this->e_v_integral_ +
  ...
  void state_update(const sim_backend::msg::VehicleState &state_msg) {
    ...
    // v_act_ = state_msg.dx_c * cos(current_pose_.psi) + state_msg.dy_c *
    // sin(current_pose_.psi);
  }
  ...
  double get_target_steering() {
    ...
    */
    ...
    // Find target point (xp_l, yp_l) up the path after (xp_c, yp_c) one
    // lookahead distance from (x_c, y_c)
    for (size_t i = 0; i < ref_points_.size(); i++) {
      ...
      RCLCPP_INFO_STREAM(this->get_logger(),
                         "Distance: " << distance_to_ref_point
                                      << ", Lookahead: " << this->l_d_);
      ...
      if (distance_to_ref_point < this->l_d_) {
        ...
        // As soon as there is a point outside the lookahead circle, set it to
        // be target point 2 and break out of loop
      } else {
        lookahead_idx_target_2 = i;
        RCLCPP_INFO_STREAM(this->get_logger(),
                           "Breaking out of loop with idx_1="
                               << lookahead_idx_target_1
                               << " and idx_2=" << lookahead_idx_target_2);
        break;
      }
    }
    ...
    double c_quad_eq =
        pow(x_2 - x_c, 2) + pow(y_2 - y_c, 2) - pow(this->l_d_, 2);

    // Two possible solutions to quadratic equation
    double t_line_1 =
        (-b_quad_eq + sqrt(pow(b_quad_eq, 2) - 4 * a_quad_eq * c_quad_eq)) /
        (2 * a_quad_eq);
    double t_line_2 =
        (-b_quad_eq - sqrt(pow(b_quad_eq, 2) - 4 * a_quad_eq * c_quad_eq)) /
        (2 * a_quad_eq);
    ...
    if (t_line_1 <= 1.0 && t_line_1 >= 0.0) {
      ...
      RCLCPP_ERROR_STREAM(
          this->get_logger(),
          "Could not find line-circle intersection for PP controller!");
    }

    // In case no point outside lookahead circle found
    if (lookahead_idx_target_2 == 0) {
      RCLCPP_ERROR_STREAM(this->get_logger(),
                          "No target point outside lookahead distance found!");
      // Use target 1
      t = 1.0;
    }
    ...
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Target Point: x=" << target_point_I.x
                                          << ", y=" << target_point_I.y);
    ...
    target_point_V.x =
        delta_x_I * cos(current_pose_.psi) + delta_y_I * sin(current_pose_.psi);
    target_point_V.y = -delta_x_I * sin(current_pose_.psi) +
                       delta_y_I * cos(current_pose_.psi);
