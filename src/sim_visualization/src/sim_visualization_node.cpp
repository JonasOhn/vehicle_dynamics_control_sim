...
public:
  SimVisualization()
    ...
    */
    ...
    state_subscription_ =
        this->create_subscription<sim_backend::msg::VehicleState>(
            "vehicle_state", 10,
            std::bind(&SimVisualization::visualize_velvec, this,
                      std::placeholders::_1));

    // Subscriber for track middle points
    middle_path_subscription_ =
        this->create_subscription<sim_backend::msg::Point2DArray>(
            "track_points2d", 10,
            std::bind(&SimVisualization::update_middle_path_visual, this,
                      std::placeholders::_1));

    // Subscriber for track left boundary points
    trackbounds_left_subscription_ =
        this->create_subscription<sim_backend::msg::Point2DArray>(
            "trackbounds_left_points2d", 10,
            std::bind(&SimVisualization::update_trackbounds_left_visual, this,
                      std::placeholders::_1));

    // Subscriber for track right boundary points
    trackbounds_right_subscription_ =
        this->create_subscription<sim_backend::msg::Point2DArray>(
            "trackbounds_right_points2d", 10,
            std::bind(&SimVisualization::update_trackbounds_right_visual, this,
                      std::placeholders::_1));

    // Subscriber for reference path ahead of vehicle
    ref_path_subscription_ =
        this->create_subscription<sim_backend::msg::Point2DArray>(
            "reference_path_points2d", 10,
            std::bind(&SimVisualization::update_ref_path_visual, this,
                      std::placeholders::_1));

    // Subscriber for MPC spline fit
    mpc_spline_subscription_ =
        this->create_subscription<sim_backend::msg::Point2DArray>(
            "mpc_spline_points2d", 10,
            std::bind(&SimVisualization::update_mpc_spline_visual, this,
                      std::placeholders::_1));

    // Subscriber for MPC prediction trajectory (to be published as marker array
    // with purple cubes)
    mpc_xy_predict_subscription_ =
        this->create_subscription<sim_backend::msg::Point2DArray>(
            "mpc_xy_predict_trajectory", 10,
            std::bind(&SimVisualization::update_mpc_xy_prediction_visual, this,
                      std::placeholders::_1));
    ...
    */
    ...
    track_publisher_PCL_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>("track_pcl", 10);

    // reference points from path planning (red dots)
    ref_path_publisher_PCL_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "reference_path_pcl", 10);

    // Track Boundary Publishers (blue and yellow vertical cylinders)
    trackbounds_left_publisher_markers_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>(
            "trackbounds_left_markers", 10);
    trackbounds_right_publisher_markers_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>(
            "trackbounds_right_markers", 10);

    // Velocity Vector Publisher (green arrow with alpha channel for
    // transparency)
    velocity_vector_publisher_ =
        this->create_publisher<visualization_msgs::msg::Marker>(
            "velocity_vector_marker", 10);

    // MPC Spline Fit Publisher (green dots)
    mpc_spline_publisher_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>("mpc_spline_pcl",
                                                              10);

    // MPC x,y prediction Publisher (purple cubes)
    mpc_xy_prediction_publisher_markers_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>(
            "mpc_xy_prediction_markers", 10);
...
private:
  void update_mpc_xy_prediction_visual(
      const sim_backend::msg::Point2DArray &point2d_array) {
  ...
  void
  update_ref_path_visual(const sim_backend::msg::Point2DArray &point2d_array) {
  ...
  void update_mpc_spline_visual(
    ...
    mpc_spline_cloud.points.resize(mpc_spline_cloud.height *
                                   mpc_spline_cloud.width);
  ...
  void update_middle_path_visual(
      const sim_backend::msg::Point2DArray &point2d_array) {
    // Create a point cloud message
  ...
  void update_trackbounds_left_visual(
      const sim_backend::msg::Point2DArray &point2d_array) {
    // Create a marker array message
  ...
  void update_trackbounds_right_visual(
      const sim_backend::msg::Point2DArray &point2d_array) {
    // Create a marker array message
