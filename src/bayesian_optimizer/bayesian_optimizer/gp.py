...
class GaussianProcess:
    ...
    def squared_exponential_kernel(self, X1, X2):
        ...
        for i in range(N1):
            for j in range(N2):
                K[i, j] = self.output_variance_ * np.exp(
                    -1
                    / (2 * (self.lengthscale_**2))
                    * np.linalg.norm(X1[i, :] - X2[j, :], 2) ** 2
                )
    ...
    def add_data(self, x: np.array, y: np.array):
        ...
        assert (
            x.shape[0] == y.shape[0]
        ), "Number of data points in input x ({0}) and output y ({1}) must match!".format(
            x.shape[0], y.shape[0]
        )
        if not self.has_data:
            ...
            ), "Trying to add input data of dimension {0} to a GP with input data of dimemsion {1}".format(
                x.shape[1], self.x_dim
            )
    ...
    def predict(self, x):
        ...
        assert (
            x.shape[1] == self.x_dim
        ), "input data dimension {0} does not match the GP input data dimension {1}".format(
            x.shape[1], self.x_dim
        )

        inv_K_noise = np.linalg.inv(
            self.squared_exponential_kernel(self.data_x_, self.data_x_)
            + self.noise_covariance_**2 * np.eye(len(self.data_x_))
        )
        ...
        std = np.sqrt(
            np.diagonal(self.squared_exponential_kernel(x, x) - k @ inv_K_noise @ k.T)
        ).reshape(-1, 1)
