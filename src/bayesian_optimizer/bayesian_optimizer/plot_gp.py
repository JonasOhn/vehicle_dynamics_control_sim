...
def plot_gp_and_acquisition_function(gp_mean, q_bounds):
    ...
    assert np.shape(q_bounds) == (3, 2) or np.shape(q_bounds) == (
        2,
        2,
    ), "Bounds have to be box bounds in 3D or 2D"
    ...
    sc = ax.scatter(
        ...
        c=self.q_hat,
        cmap="rainbow",
        vmin=np.min(self.q_hat),
        vmax=np.max(self.q_hat),
    )
    sc_star = ax.scatter(
...
plot_acquisition_function(
    bayesian_optimizer, Q, lb_q_sd, ub_q_sd, lb_q_n, ub_q_n, lb_q_mu, ub_q_mu
)
