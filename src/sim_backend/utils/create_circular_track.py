...
for angle in angles:
    ...
    left_blue_cones_positions.append(
        [
            (radius - trackwidth / 2.0) * np.cos(angle),
            radius - (radius - trackwidth / 2.0) * np.sin(angle),
        ]
    )
    right_yellow_cones_positions.append(
        [
            (radius + trackwidth / 2.0) * np.cos(angle),
            radius - (radius + trackwidth / 2.0) * np.sin(angle),
        ]
...
with open(
    "src/sim_backend/tracks/circular_track_blue_cones.csv", "w", newline=""
) as csv_file:
...
with open(
    "src/sim_backend/tracks/circular_track_yellow_cones.csv", "w", newline=""
) as csv_file:
    csv_writer = csv.writer(csv_file, delimiter=",")
...
with open(
    "src/sim_backend/tracks/circular_track_middle_path.csv", "w", newline=""
) as csv_file:
    csv_writer = csv.writer(csv_file, delimiter=",")
