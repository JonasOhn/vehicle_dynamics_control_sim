  ...
  SimTimeNode()
    ...
    if (this->rtf_ <= 1.0) {
      ...
      RCLCPP_WARN_STREAM(this->get_logger(),
                         "Real time factor must be smaller than 1. Setting "
                         "real time factor to 1.");
      this->rtf_ = 1.0;
      this->dt_ms_ = 1;
    }
    ...
    sim_timer_ = this->create_wall_timer(
        this->dt_, std::bind(&SimTimeNode::publish_time, this));
...
private:
  void publish_time() {
    ...
    // If the nanoseconds are greater than 1e9, reset the nanoseconds and add 1
    // to the seconds
