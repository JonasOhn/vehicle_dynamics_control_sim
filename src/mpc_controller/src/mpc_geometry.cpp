...
int MpcGeometry::get_number_of_spline_evaluations() {
  std::cout << "Getting length of knot vector: " << this->n_t_evals_
            << std::endl;
 ...
 */
int8_t
MpcGeometry::set_control_points(std::vector<std::vector<double>> &waypoints) {
  ...
  ref_point[0] = 2 * this->ref_points_[this->ref_points_.size() - 1][0] -
                 this->ref_points_[this->ref_points_.size() - 2][0];
  ref_point[1] = 2 * this->ref_points_[this->ref_points_.size() - 1][1] -
                 this->ref_points_[this->ref_points_.size() - 2][1];
  ...
  for (size_t i = 0; i < (size_t)this->t_ref_spline_.size(); i++) {
    ...
    for (this->i_ = 0; this->i_ < 2; this->i_++) {
      ...
      for (this->k_ = 0; this->k_ < 4; this->k_++) {
        this->ctrl_points_(this->i_, this->k_) =
            this->ref_points_[this->t_int_ + this->k_][this->i_];
      }
    }
    ...
    this->t_vec_ << pow(this->t_frac_, 3), pow(this->t_frac_, 2), this->t_frac_,
        1.0;
    // fill 1st deriv knot polynomial vector
    this->dt_vec_ << 3 * pow(this->t_frac_, 2), 2 * this->t_frac_, 1.0, 0.0;
    // fill 2nd deriv knot polynomial vector
    this->ddt_vec_ << 6 * this->t_frac_, 2.0, 0.0, 0.0;
    this->xy_ref_spline_.push_back(this->ctrl_points_ * this->bspline_coeff_ *
                                   this->t_vec_);
    this->dxy_ref_spline_.push_back(this->ctrl_points_ * this->bspline_coeff_ *
                                    this->dt_vec_);
    this->ddxy_ref_spline_.push_back(this->ctrl_points_ * this->bspline_coeff_ *
                                     this->ddt_vec_);

    // Discretely integrate up the path coordinate s
    if (i > 0) {
      s_ref_length +=
          sqrt(pow(this->xy_ref_spline_[i][0] - this->xy_ref_spline_[i - 1][0],
                   2.0) +
               pow(this->xy_ref_spline_[i][1] - this->xy_ref_spline_[i - 1][1],
                   2.0));
    }
    ...
    curv_instant /= (pow((pow(this->dxy_ref_spline_[i][0], 2.0) +
                          pow(this->dxy_ref_spline_[i][1], 2.0)),
                         1.5));
...
double MpcGeometry::get_mpc_curvature(double s_to_eval) {
  ...
  // for first entry that is larger than in mpc s ref, interpolate between this
  // and the previous point
  for (this->j_ = 0; this->j_ < (int)this->s_ref_spline_.size(); this->j_++) {
    // As soon as s_to_eval (input) is larger than s_ref from spline, calculate
    // curvature as linear interpolation and break out
    if (s_to_eval <= this->s_ref_spline_[this->j_]) {
      idx_next_s = this->j_;
      // Linearly interpolate between given s_ref values on the spline to get
      // curvature
      curv_ref_return = this->curv_ref_spline_[idx_next_s - 1] +
                        (s_to_eval - this->s_ref_spline_[idx_next_s - 1]) /
                            (this->s_ref_spline_[idx_next_s] -
                             this->s_ref_spline_[idx_next_s - 1]) *
                            (this->curv_ref_spline_[idx_next_s] -
                             this->curv_ref_spline_[idx_next_s - 1]);

      break;
    }
  }
  std::cout << "Got MPC curvature at s = " << s_to_eval << " to be "
            << curv_ref_return << std::endl;
 ...
 * @return 0 if successful
 */
int8_t MpcGeometry::get_spline_eval_waypoint(std::vector<double> &vec,
                                             int idx) {
