...
void MpcController::reset_prediction_trajectories() {
  ...
  for (this->i_ = 0; this->i_ <= this->horizon_params_.N_horizon_mpc;
       this->i_++) {
...
int8_t MpcController::set_model_parameters(double l_f, double l_r, double m,
...
int8_t MpcController::set_cost_parameters(double q_sd, double q_n, double q_mu,
                                          double q_ax, double q_dels,
                                          double r_dax, double r_ddels) {
...
int8_t MpcController::set_solver_parameters(int sqp_max_iter, int rti_phase,
...
int8_t MpcController::init_solver() {
  ...
  int creation_status = veh_dynamics_ode_acados_create_with_discretization(
      this->acados_ocp_capsule_, this->horizon_params_.N_horizon_mpc, NULL);
  if (creation_status) {
    std::cout << "Initializing NLP solver capsule failed." << std::endl;
    return 1;
  }
  this->nlp_config_ =
      veh_dynamics_ode_acados_get_nlp_config(this->acados_ocp_capsule_);
  this->nlp_dims_ =
      veh_dynamics_ode_acados_get_nlp_dims(this->acados_ocp_capsule_);
  this->nlp_in_ = veh_dynamics_ode_acados_get_nlp_in(this->acados_ocp_capsule_);
  this->nlp_out_ =
      veh_dynamics_ode_acados_get_nlp_out(this->acados_ocp_capsule_);
  this->nlp_solver_ =
      veh_dynamics_ode_acados_get_nlp_solver(this->acados_ocp_capsule_);
  this->nlp_opts_ =
      veh_dynamics_ode_acados_get_nlp_opts(this->acados_ocp_capsule_);

  // Solver options set
  ocp_nlp_solver_opts_set(this->nlp_config_, this->nlp_opts_, "rti_phase",
                          &(this->solver_params_.rti_phase));
  ocp_nlp_solver_opts_set(this->nlp_config_, this->nlp_opts_, "max_iter",
                          &(this->solver_params_.sqp_max_iter));
  ocp_nlp_solver_opts_set(this->nlp_config_, this->nlp_opts_,
                          "warm_start_first_qp",
                          &(this->solver_params_.warm_start_first_qp));

  return 0;
}

int8_t MpcController::set_state(double x_c, double y_c, double psi,
                                double vx_local, double dels, double ax,
                                double &s, double &n, double &mu) {
  int path_idx =
      this->mpc_geometry_obj_.get_initial_path_reference_idx(x_c, y_c);

  std::cout << "Setting state." << std::endl;
  // s: Progress coordinate
  this->x_[0] =
      this->mpc_geometry_obj_.get_initial_progress(x_c, y_c, path_idx); // s
  s = this->x_[0];
  std::cout << "Setting s = " << s << std::endl;

  // n: Lateral deviation coordinate
  this->x_[1] = this->mpc_geometry_obj_.get_initial_lateral_deviation(
      x_c, y_c, path_idx); // n
  n = this->x_[1];
  std::cout << "Setting n = " << n << std::endl;

  // mu: Heading difference from path
  this->x_[2] = this->mpc_geometry_obj_.get_initial_heading_difference(
      psi, path_idx); // mu
  mu = this->x_[2];
...
int8_t MpcController::set_initial_state() {
  ...
  ocp_nlp_constraints_model_set(this->nlp_config_, this->nlp_dims_,
                                this->nlp_in_, 0, "idxbx", idxbx0);
  ocp_nlp_constraints_model_set(this->nlp_config_, this->nlp_dims_,
                                this->nlp_in_, 0, "lbx", bx0);
  ocp_nlp_constraints_model_set(this->nlp_config_, this->nlp_dims_,
                                this->nlp_in_, 0, "ubx", bx0);
...
int8_t MpcController::init_mpc_parameters() {
  ...
  for (this->i_ = 0; this->i_ < this->horizon_params_.N_horizon_mpc;
       this->i_++) {
    std::cout << "Setting kappa at stage i = " << this->i_
              << " based on s = " << this->x_traj_[this->i_][0] << std::endl;
    // curvature value at stage i_ from geometry object, given s
    this->curv_ref_mpc_[this->i_] =
        this->mpc_geometry_obj_.get_mpc_curvature(this->x_traj_[this->i_][0]);
    ...
    std::cout << "Model parameter kappa = " << this->curv_ref_mpc_[this->i_]
              << " at s = " << this->x_traj_[this->i_][0] << std::endl;

    veh_dynamics_ode_acados_update_params(this->acados_ocp_capsule_, this->i_,
                                          p, NP);
  }
...
int8_t MpcController::init_mpc_horizon() {
  ...
  for (this->i_ = 0; this->i_ <= this->horizon_params_.N_horizon_mpc - 2;
       this->i_++) {
    ...
    ocp_nlp_out_set(this->nlp_config_, this->nlp_dims_, this->nlp_out_, i_, "x",
                    this->x_stage_to_fill_);
    ...
    ocp_nlp_out_set(this->nlp_config_, this->nlp_dims_, this->nlp_out_, i_, "u",
                    this->u_stage_to_fill_);
  }

  // take u_previous_(N - 1) from last iteration to also fill u_new_(N - 1)
  ocp_nlp_out_set(this->nlp_config_, this->nlp_dims_, this->nlp_out_,
                  this->horizon_params_.N_horizon_mpc - 1, "u",
                  this->u_stage_to_fill_);
  ...
  // take x_previous_(N) from last prediction to fill x_new_(N - 1) and
  // x_new_(N)
  ocp_nlp_out_set(this->nlp_config_, this->nlp_dims_, this->nlp_out_,
                  this->horizon_params_.N_horizon_mpc - 1, "x",
                  this->x_stage_to_fill_);
  ocp_nlp_out_set(this->nlp_config_, this->nlp_dims_, this->nlp_out_,
                  this->horizon_params_.N_horizon_mpc, "x",
                  this->x_stage_to_fill_);
...
int8_t MpcController::get_solver_statistics(int &nlp_solv_status,
                                            int &qp_solv_status, int &sqp_iter,
                                            double &kkt_norm,
                                            double &solv_time) {
  int8_t return_val = 0;
...
int8_t MpcController::update_solver_statistics() {
  ...
  // get time
  ocp_nlp_get(this->nlp_config_, this->nlp_solver_, "time_tot",
              &(this->solver_out_.solve_time));

  // get stage 0 KKT inf. norm
  ocp_nlp_out_get(this->nlp_config_, this->nlp_dims_, this->nlp_out_, 0,
                  "kkt_norm_inf", &(this->solver_out_.kkt_norm_inf));

  // get SQP iterations
  ocp_nlp_get(nlp_config_, this->nlp_solver_, "sqp_iter",
              &(this->solver_out_.sqp_iterations));

  // Get QP status
  ocp_nlp_get(this->nlp_config_, this->nlp_solver_, "qp_status",
              &(this->solver_out_.qp_solver_status));
...
int8_t MpcController::solve_mpc() {
  ...
  this->solver_out_.nlp_solver_status =
      veh_dynamics_ode_acados_solve(this->acados_ocp_capsule_);
...
int8_t MpcController::get_input(double (&u)[2]) {
  ...
  if (this->solver_out_.nlp_solver_status == 0 ||
    ...
    ocp_nlp_out_get(this->nlp_config_, this->nlp_dims_, this->nlp_out_,
                    stage_to_eval, "x", &x_to_eval);
...
int8_t MpcController::get_predictions(
  ...
  for (int i = 0; i < this->horizon_params_.N_horizon_mpc; i++) {
    ocp_nlp_out_get(this->nlp_config_, this->nlp_dims_, this->nlp_out_, i, "x",
                    &this->x_traj_[i]);
    ocp_nlp_out_get(this->nlp_config_, this->nlp_dims_, this->nlp_out_, i, "u",
                    &this->u_traj_[i]);
    ...
    dpsi_predict.push_back(tan(this->x_traj_[i][5]) * this->x_traj_[i][3] /
                           this->model_params_.l_r);

    daxm_predict.push_back(this->u_traj_[i][0]);
    ddels_predict.push_back(this->u_traj_[i][1]);

    kappa_traj_mpc.push_back(
        this->mpc_geometry_obj_.get_mpc_curvature(this->x_traj_[i][0]));
  ...
  // get terminal state
  ocp_nlp_out_get(this->nlp_config_, this->nlp_dims_, this->nlp_out_,
                  this->nlp_dims_->N, "x", &this->x_traj_[this->nlp_dims_->N]);
  ...
  vy_predict.push_back(tan(this->x_traj_[this->nlp_dims_->N][5]) *
                       this->x_traj_[this->nlp_dims_->N][3]);
  dpsi_predict.push_back(tan(this->x_traj_[this->nlp_dims_->N][5]) *
                         this->x_traj_[this->nlp_dims_->N][3] /
                         this->model_params_.l_r);
...
/**
 * Getter function for the fitted spline and the predicted positions along MPC
 * horizon
 *
 ...
 */
int8_t MpcController::get_visual_predictions(
    std::vector<std::vector<double>> &xy_spline_points,
  ...
  for (i = 0; i < this->mpc_geometry_obj_.get_number_of_spline_evaluations();
       i++) {
  ...
  for (i = 0; i < this->horizon_params_.N_horizon_mpc; i++) {
    ...
    xy_predicted_point[0] =
        xy_predicted_point[0] + (dx * cos(psi_predict) - dy * sin(psi_predict));
    xy_predicted_point[1] =
        xy_predicted_point[1] + (dy * cos(psi_predict) + dx * sin(psi_predict));
 ...
 */
int8_t
MpcController::set_reference_path(std::vector<std::vector<double>> &path) {
...
double MpcController::get_mass() { return this->model_params_.m; }
