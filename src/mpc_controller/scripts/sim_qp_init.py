import numpy as np
from setup_qp_init import setup_ocp_init, load_mpc_initializer_yaml_params
from plot_sim_qp_init import plot_qp_dynamics
...
def main(use_stepped_sim:bool=False):
    model_params, horizon_params, _, _ = load_mpc_initializer_yaml_params()
    ...
    ocp_solver, integrator = setup_ocp_init(x0, simulate_ocp=True)
    ...
    for i in range(Nsim):
        ...
        if use_stepped_sim and i > stepping_start_idx:
            ...
            plot_qp_dynamics(np.linspace(0, (horizon_params['T_final']/horizon_params['N_horizon'])*i, i+1),
                                idx_b_x, lb_x, ub_x,
                                idx_b_u, lb_u, ub_u, 
                                simU[:i, :], simX[:i+1, :],
    ...
    plot_qp_dynamics(np.linspace(0, (horizon_params['T_final']/horizon_params['N_horizon'])*Nsim, Nsim+1),
                idx_b_x, lb_x, ub_x,
                idx_b_u, lb_u, ub_u, 
                simU, simX)
